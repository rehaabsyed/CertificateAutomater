...
\begin{enumerate}
    ...
    \item \mintinline{shell}{git rm <files> --cached}, removes added files from the current branch in the tree
    \begin{enumerate}
        \item Does not delete the files
        \item To delete the file(s) and remove it from Git's tree, use \mintinline{shell}{git rm <files> -f}
...
\mint[linenos=false, frame=none]{shell}|.\venv\Scripts\activate.bat|

\noindent
on Windows, or

\mint[linenos=false, frame=none]{shell}|source ./bin/activate|
...
\inputminted[linenos=false]{shell}{python_examples/pytest_simple_output.txt}
