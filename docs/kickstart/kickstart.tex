...
%----------------------------------------------------------------------------------------
%    PACKAGES AND OTHER DOCUMENT CONFIGURAT\acrshort{io}NS
%----------------------------------------------------------------------------------------
...
\SetWatermarkText{\textsf{\textbf{DRAFT 6}}}
...
%----------------------------------------------------------------------------------------
%    ASSIGNMENT INFORMAT\acrshort{io}N
%----------------------------------------------------------------------------------------
...
\begin{enumerate}
    ...
    \begin{enumerate}
        \item Strongly recommended to use \mintinline{python}{argparse} \gls{library}.
    \end{enumerate}
...
\begin{enumerate}
    ...
    \begin{enumerate}
        ...
        \item Strongly recommended to use the \mintinline{python}{logging} \gls{library}.
    \end{enumerate}
...
\begin{info}[\acrshort{http} Status Codes]
...
In \Gls{python}, encapsulation is implemented very differently to other \acrshort{oop} languages. Languages like Java, C\#, and C++ have the keywords \mintinline{c++}{public}, \mintinline{c++}{protected}, and \mintinline{c++}{private} for deciding how hidden a variable or method is. \mintinline{c++}{public} simply means anything that has access to the \gls{class} or \gls{object} can access the member, be it a variable or method. \mintinline{c++}{private}, however, means only the \gls{class} or \gls{object} can access the member, completely hiding it from all other code. While useful for data protection, it was quickly found that \mintinline{c++}{private} is too restrictive for many applications. Instead, \mintinline{c++}{protected} has replaced \mintinline{c++}{private}, as it allows subclasses to access the member, dicussed further in the inheritance subsection below. \Gls{python} does not have these keywords, instead opting for using underscores: no underscores delcares the member (or ``attribute'' as it is known in \Gls{python}) as public, a single underscore means protected, and a double underscore, or ``\gls{dunder}'', means private. Protected and private variables can then be read using the \mintinline{python}{@property} decorator and set using the \mintinline{python}{@x.setter} decorator, where \mintinline{python}{x} is the variable being set. Note the example below and the lack of \glspl{dunder}.
...
\begin{figure}[b!]
    \centering
    \includegraphics[width=0.6\textwidth]{figures/cost-time-quality_triangle.png}
...
\begin{enumerate}
    ...
    \begin{enumerate}
        ...
        \item Wildcards are accepted for \texttt{<files>}, e.g. \mintinline{shell}{git add src/*.py} means add all \Gls{python} source files in the \texttt{src/} directory
    \end{enumerate}
...
In \Gls{python}, using \glspl{venv} is handled via the \texttt{virtualenv} package. Once installed with \mintinline{shell}{pip} or \mintinline{shell}{conda}, the virtual environment can be created in the current working directory via

\mint[linenos=false, frame=none]{shell}|python -m venv <name>|

\noindent
where \mintinline{shell}{<name>} is the name of the \gls{venv}, typically \mintinline{shell}{venv}. A file system will be created with an activiate file. To start the virtual environment, simply type
...
\begin{enumerate}
    ...
    \item Function and variable names should be in \mintinline{shell}{camelCase} or \mintinline{shell}{snake_case}.
...
\begin{itemize}
    ...
    \item The first parameter of an \gls{object} method should be \mintinline{python}{self}
    \item The first parameter of a \gls{class} method should be \mintinline{python}{cls}
...
In \Gls{python}, \glspl{docstring} attempt to solve this issue via simple triple quote comments at the start of a class, function, method, or module. A docstring, at a minimum, is comprised of a single line summarises what is being documented \cite{python-docstring}. Additionally, all \glspl{docstring} may also contain a more detailed description, with a blank line inbetween the summary and description. Functions and methods can also have one line per parameter and return value, detailing their names, datatypes, and purpose. \Gls{class} \glspl{docstring} should also have the attributes (i.e. the publicly accessible \gls{object} variables) and public methods detailed. The example below shows a simple example of an incomplete \mintinline{python}{ComplexNumber} \gls{class}. Once complete, the \mintinline{python}{__doc__} property or the \mintinline{python}{help()} function can be used to display the docstring without finding and reading the \gls{src-code}.
...
To use \Gls{sphinx}, a few simple steps must be followed. First, the \glspl{docstring} must be written for all public constants, classes, methods, and functions. These \glspl{docstring} must either be writtin in the \Gls{sphinx}, Google, or NumPy format (the latter two require the \mintinline{python}{napoleon} extension) \cite{sphinx-napolean}. Next, \Gls{sphinx} must be installed via \mintinline{shell}{pip install sphinx}. After, the command \mintinline{shell}{sphinx-quickstart} can be run to automate most of the \texttt{conf.py} file, which is the settings file for \Gls{sphinx} when generating documentation.\footnote{It is strongly recommend to run this command in a \texttt{docs/} folder} Then, the reStructedText files can be generated via

\mint[linenos=false, frame=none]{shell}|sphinx-apidoc -o . ../src|

\noindent
Note that the \mintinline{shell}{-o .} tells \Gls{sphinx} to create the files in the current working directory (e.g. \mintinline{shell}{docs/}), and \mintinline{shell}{../src} tells \Gls{sphinx} where to look for \gls{src-code} files. Finally, running \mintinline{shell}{make html} tells \Gls{sphinx} to use the reStructedText files to generate \acrshort{html} files.

If the \glspl{docstring} are updated, then the command \mintinline{shell}{make html} can be rerun to update the \acrshort{html} files. Fortunately, the reStructedText files refer to the \glspl{docstring} instead of copying them, meaning they do not need to be recreated. For Mac and Linux \gls{user}s, the \Gls{make} program should already be installed \cite{makefile}. For Windows users, \Gls{make} can be installed via installing Chocolatey first, then running \mintinline{shell}{choco install make} \cite{chocolatey}.
...
Multiple \glspl{framework} exist for \Gls{python} to make test scripts smarter, thorough, and easy to use. Of these \glspl{framework}, \mintinline{python}{pytest} has proven the simplest and easiest to use without sacrificing test thoroughness \cite{pytest}. By simply overriding \Gls{python}'s \mintinline{python}{assert} statement, \mintinline{python}{pytest} allows developers to write simple code in the form of functions, at least one per test case. Below is a simple script writtin using \mintinline{python}{pytest} \gls{framework}:
...
Fortunately, \Gls{python} already has a mocking \gls{library} for this exact purpose \cite{python-mock}. Mocking allows test cases to create mock versions of functions and classes. Furthermore, the \mintinline{python}{pytest-mock} extension for \mintinline{python}{pytest} provides simple wrappers for the mocking \gls{library} \cite{pytest-mock}. Most importantly, functions are mocked by simply using \mintinline{python}{mocker.patch(target, return_value=val)}
...
\begin{itemize}
    ...
    \item \Gls{python} \mintinline{python}{assert} documentation: \url{https://docs.python.org/3/reference/simple_stmts.html#the-assert-statement}
