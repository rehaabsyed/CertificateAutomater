...
\begin{enumerate}
    ...
    \begin{enumerate}
        ...
        \item Key names should use hypens or (prefferably) underscores instead of spaces.
...
While useful for prototyping and developers, CLI have not been suitable for the average computer user for decades. Instead, graphical user interfaces (GUI) are the norm. Whereas CLIs are sequential, always following some order of IO actions in a linear of tree structure, GUIs are event driven. In GUIs, the structure of the interface remains constant until an event occurs, such as the user selecting a button, saying ``Hay, Siri'', or a message being received over the internet. GUIs achieve their event driven nature via heavy usage of callbacks and OOP: an object is given the callback, then sits and waits until an event or ``trigger'' occurs. Once the trigger activates, the object uses the callback to determine what to do.  Separating the when (the object) from the what (the callback) allows for reusable and small code.
...
\begin{enumerate}
    \item \mintinline{console}{git clone <url> -b <branch>}, clone branch \texttt{<branch>} from the repository \texttt{<url>}
    \item \mintinline{console}{git branch <name>}, create a branch with name \texttt{<name>} from the current branch
    \item \mintinline{console}{git checkout <name>}, switch to the branch \texttt{<name>}
    \item \mintinline{console}{git add <files>}, command Git to start tracking the files \texttt{<files>}
    \begin{enumerate}
        \item Once files have been added, they are referred to as ``staged''
        \item Wildcards are accepted for \texttt{<files>}, e.g. \mintinline{console}{git add src/*.py} means add all Python source files in the \texttt{src/} directory
    \end{enumerate}
    \item \mintinline{console}{git commit}, creates a snapshot of the staged files
    \begin{enumerate}
        \item Will open a text file in Vim to write a short message about the commit changes
        \item Can skip Vim usage via \mintinline{console}{git commit -m "changes made"}
    \end{enumerate}
    \item \mintinline{console}{git push}, uploads all new commits in current branch to remote repository
\end{enumerate}
...
\begin{enumerate}
    \item \mintinline{console}{git pull}, updates current working branch with new commits from remote repository
    \item \mintinline{console}{git branch}, displays all branches in the local repository
    \item \mintinline{console}{git merge <branch>}, combine the history from branch \texttt{<branch>} into current working branch
    \item \mintinline{console}{git merge}, combines remote repository into current local branch
    \item \mintinline{console}{git log}, Display version history for the current branch
    \item \mintinline{console}{git diff <branch1> <branch2>}, compare differences between two branches
    \item \mintinline{console}{git status}, displays the current working branch, how many commits the branch is from the master branch, what files have been staged, and what modified files have not been staged
    \item \mintinline{console}{git revert HEAD}, undos the last commit in the current working branch
    \begin{enumerate}
        \item Note that \texttt{HEAD} refers to the current working branch
    \end{enumerate}
    \item \mintinline{console}{git reset <commit>}, undoes all commits from \texttt{<commit>} onwards
    \item \mintinline{console}{git rm <files> --cached}, removes added files from the current branch in the tree
    \begin{enumerate}
        \item Does not delete the files
        \item To delete the file(s) and remove it from Git's tree, use \mintinline{console}{git rm <files> -f}
        \item If a file was deleted without using \mintinline{console}{git rm <files>}, Git will complain about missing files
        \item To remove a deleted file from Git's tree, use \mintinline{console}{git commit -a}
    \end{enumerate}
    \item \mintinline{console}{git blame <file>}, who changed what in \texttt{<file>} and when
\end{enumerate}
...
\mint{console}|python -m venv <name>|
...
\inputminted[linenos=true]{python}{python_examples/pytest_simple.py}

\noindent
Note that every function starts with \mintinline{python}{test_} in their name. This is required for \mintinline{python}{pytest} to recognise the function as a test case that needs to be run. To run the test script, type \mintinline{console}{pytest <filename>} in the console. For the example above, the output will be 

\inputminted[linenos=true]{shell}{python_examples/pytest_simple_output.txt}
...
\inputminted[linenos=true]{python}{python_examples/pytest_fixture.py}

\noindent
Similarly, if multiple sets of data need to be tested against the same test, a parameterised test can be used:

\inputminted[linenos=true]{python}{python_examples/pytest_parameterize.py}

\noindent
Lastly, \mintinline{python}{pytest} can run test cases in parallel by installing the \mintinline{console}{pytest-xdist} plugin, then running \mintinline{python}{pytest} with an \texttt{-n} flag, specifying the number of processes to be created. For example,

\mint{shell}{pytest test_script.py -n 6}
