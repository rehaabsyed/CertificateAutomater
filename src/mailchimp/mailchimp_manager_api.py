...
class MailchimpManagerApiKey:
    """
    Mailchimp manager using API keys.

    Unpreferred way of accessing user Mailchimp accounts. Should only be used
    for a prototype. Use OAuth 2 for a more secure authorisation.
    ...
    def authorise(self, api_key: str, server_key: str):
        """
        Authorise self to use the user's Mailchimp account.

        Args:
            api_key (str): The API key provided by Mailchimp.
            server_key (str): The server key provided by Mailchimp.
        """
        raise NotImplementedError
